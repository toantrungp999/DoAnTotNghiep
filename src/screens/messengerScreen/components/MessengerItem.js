...
class MessengerItem extends Component {
    ...
    render() {
        ...
        if (isMessengerItemToAdmin) {
            ...
            image = !messenger.user1 || !messenger.user2 ? "" : { uri: (messenger.user1._id === userId ? messenger.user2.image : messenger.user1.image) };
            name = !messenger.user1 || !messenger.user2 ? "Chăm sóc khác hàng" : (messenger.user1._id === userId ? messenger.user2.name : messenger.user1.name);
        }
...
const styles = StyleSheet.create({
    messengerItem: {
        flexDirection: 'row', width: '100%', padding: 6, marginBottom: 2
    },
    name: {
        fontSize: 14,
        fontWeight: "500"
    ...
    messengerValue: {
        flexDirection: 'row', marginTop: 3, width: "100%"
    },
})
