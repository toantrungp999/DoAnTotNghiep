    ...
    return axios({
        ...
        if (response.status === ResponeseStatus.INPUT_DATA) {
            if (response.message.includes("length must be less than or equal to"))
                response.message = response.message.replace("length must be less than or equal to", Message.CHECK_LENGTH_MAX_VN).replace("characters long", Message.CHARACTER_VN);
            if (response.message.includes("length must be at least"))
                response.message = response.message.replace("length must be at least", Message.CHECK_LENGTH_MIN_VN).replace("characters long", Message.CHARACTER_VN);
        }
        return response;
    }).catch(error => {
        let status = 404;
...
function callApiToken(dispatch, endpoint, method = 'GET', body = null) {
    ...
    if (!accessToken) {
        if (!refreshToken) {
            ...
            return { status: ResponeseStatus.NO_TOKEN, data: null, message: "Token not found!" };
        }
        else {
            return callApiRefreshToken(refreshToken).then(response => {
                if (!response.accessToken)
                    return (dispatch) => {
                        dispatch(logoutRequest());
                        return { status: ResponeseStatus.REFRESH_TOKEN_FAIL, data: null, message: "Refresh token fail!" };
                    }
        ...
        return callApi(endpoint, method, body, accessToken).then(response => {
            ...
            else if (response.status === ResponeseStatus.NO_PROVIDED_TOKEN || response.status === ResponeseStatus.INVALID_TOKEN) {
                dispatch(logoutRequest());
                return { status: ResponeseStatus.NO_TOKEN, data: null, message: "Invalid Token!" };
            }
            else
                return response;
        });
...
export {
    callApi, callApiToken, callApiRefreshToken
}